...
% filter method. Uses data from a log file.
%
%%
...
while feof(fileId) ~= 1
    ...
    fftCh1Log = 20*log10(fftCh1);
    
    % alpha filter detection
    fastCMap = (1-fastAlpha).*fastCMap + fastAlpha.*fftCh1;
    slowCMap = (1-slowAlpha).*slowCMap + slowAlpha.*fftCh1;
    ...
    %RDMapCh1 = imagesc(fftCh1Log(:, 1:nextPowOf2Range/2));
    ax1 = subplot(2,2,1);
    CMapSlowCh1 = imagesc(slowCMapLog(:, 1:nextPowOf2Range/2));
    title('Slow Map')
    
    ax2 = subplot(2,2,2);
    CMapFastCh1 = imagesc(fastCMapLog(:, 1:nextPowOf2Range/2));
    title('Fast Map')
    
    ax3 = subplot(2,2,3);
    differenceMap = imagesc(diffMap(:, 1:nextPowOf2Range/2));
    title('Difference Map')
    
    ax4 = subplot(2,2,4);
    detectionMap = imagesc(DetectionMatrix(:, 1:nextPowOf2Range/2));
    title('Detection Map')
